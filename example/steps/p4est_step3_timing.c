...
step3_timestep (p4est_t * p4est, double time)
{
  ...
  clock_t t_start = 0;
  clock_t t_start_compute = 0;
  clock_t t_start_adapt = 0;

  double total_time=0, compute_time=0, adapt_time=0;
  ...
  for (t = 0., i = 0; t < time; t += dt, i++) {
	...
	t_start_adapt = clock();
        ...
        ghost = NULL;
        ghost_data = NULL;
	adapt_time += (double) (clock () - t_start_adapt) / CLOCKS_PER_SEC;
  ...
  P4EST_GLOBAL_PRODUCTIONF ("TOTAL TIME is %f seconds\n", total_time);
  P4EST_GLOBAL_PRODUCTIONF ("ADAPT TIME is %f seconds\n", adapt_time);
